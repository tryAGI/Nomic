...
The Nomic SDK implements `IEmbeddingGenerator<string, Embedding<float>>` from [Microsoft.Extensions.AI](https://learn.microsoft.com/en-us/dotnet/ai/microsoft-extensions-ai) for text embeddings, plus `AIFunction` tool wrappers for use with any `IChatClient`.
